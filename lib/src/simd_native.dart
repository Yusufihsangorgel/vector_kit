// SIMD kernels for the Dart VM, where `Float32x4` compiles to real vector
// instructions. Reached through `simd.dart`; never imported directly.
//
// The scalar counterpart in `simd_web.dart` mirrors every name here. Keep the
// two signatures identical or the conditional export in `simd.dart` will fail
// on whichever platform is not being tested at the time.

import 'dart:typed_data';

/// A view over a [Float32List] in the form the kernels want.
///
/// Four-float lanes here; the plain list itself on the web. Lane indices and
/// counts passed to [dotLanes] and [squaredDistanceLanes] are in units of this
/// view, so callers do not have to know which one they are holding.
typedef Lanes = Float32x4List;
